...
(deftest comparison-operators
  ...
  (testing "Comparison with nil - three-valued: nil (unknown)"
    (is (nil? (eval-dt "5 > nil"))))

  (testing "nil > nil — both unknown, result nil"
...
(deftest nil-in-pipelines
  (testing "Nil-tolerant field access in comparison"
    ;; user.age is nil (field does not exist), nil > 18 => nil (unknown)
    (is (nil? (eval-dt-last
               "user is {name: \"Alice\"}"
               "user.age > 18"))))
